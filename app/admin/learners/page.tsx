...
import { getModuleSlugs, getModuleMeta, getLessonsForModule } from '@/lib/content'
...
export default async function LearnersPage() {
  ...
  const moduleInfo = await Promise.all(
    moduleSlugs.map(async slug => {
      ...
      try {
        lessonCount = (await getLessonsForModule(slug)).length
      } catch {}
      return { slug, title, lessonCount }
    })
  )
  ...
  const [{ data: users }, { data: completions }, { data: visits }] = await Promise.all([
    ...
    supabase.from('lesson_progress').select('user_id, module_slug, viewed_at'),
  ])
  ...
  for (const row of visits ?? []) {
    ...
    if (row.viewed_at && (!prev || row.viewed_at > prev)) {
      lastVisitByUser.set(row.user_id, row.viewed_at)
    }
  }
  ...
  const learners = (users ?? []).map(u => {
    ...
    const lastActive = [lastCheckpointByUser.get(u.id), lastVisitByUser.get(u.id)]
